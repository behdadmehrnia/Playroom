"""Core Yar Kids logic (personas, intent, textbooks, web search, generation).

Copied from the OpenWebUI ``pipe.py`` module into the standalone API so ``/api``
has no dependency on the repo-root Pipe. The OpenWebUI ``Pipe`` class and
``OpenWebUILLMClient`` are intentionally omitted.
"""
